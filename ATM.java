...
public class ATM {
    ...
    public static void start() throws CloneNotSupportedException {
        ...
        while (isRunning) {//Start a loop that continues as long as isRunning is true.
            ...
            if (choice == 1) {
                Account admin = AdminAction.login(accountList, scanner);// Attempt Admin login and return the Admin account object.
                ...
                Account user =  UserAction.login(accountList, scanner);// Attempt User login and return the User account object.
    ...
    public static void adminMenu(Account admin) { // a static method adminMenu() that takes an Account (admin) as a parameter.
        ...
        while (isRunning) { // Start a while loop that runs as long as isRunning is true.
            ...
            if (choice == 1) {
                AdminAction.addUser(scanner); // Call the addUser() method from AdminAction to add a new user.
            } else if (choice == 2) {
                System.out.println("Enter the User ID to delete:");
                String userId = scanner.nextLine();
                AdminAction.deleteUser(userId); // Call the deleteUser() method from AdminAction to delete the user with the given ID.
            } else if (choice == 3) {
                AdminAction.handleTransactionHistory(admin, scanner); // Call the handleTransactionHistory() method from AdminAction to view the transaction history.
            } else if (choice == 4) {
                AdminAction.viewAllAccounts(); //  Call the viewAllAccounts() method from AdminAction to display all accounts.
            } else if (choice == 5) {
    ...
    public static void userMenu(Account user) throws CloneNotSupportedException { // method to chose userMenu
        boolean isRunning = true;
        while (isRunning) {
            ...
            if (choice == 1) {
                ...
                UserAction.depositAmt(user); // Call the depositAmt() method from UserAction to deposit money into the account.
            } else if (choice == 3) {
                UserAction.withdrawAmt(user); // Call the withdrawAmt() method from UserAction to withdraw money from the account.
            } else if (choice == 4) {
                UserAction.viewTransactions(user); // Call the viewTransactions() method from UserAction to view the user's transaction history.
            } else if (choice == 5) {
    ...
    public static void manageATMInventory(Account admin) { // to manage ATM Inventory
        ...
        int choice = Integer.parseInt(scanner.nextLine());
        if (choice == 1) {
            AdminAction.addATMoney(admin, scanner); // Call the addATMoney() method from AdminAction to add cash to the ATM inventory.
        } else if (choice == 2) {
